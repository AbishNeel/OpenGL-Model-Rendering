...
{
	// Make a JSON object
	std::string text = Utilities::GetFileContents(file);
...
void Model::Draw(Shader& shader, Camera& camera)
{
	...
	{
		meshes[i].Mesh::Draw(shader, camera, _matricesMeshes[i], _translationsMeshes[i], _rotationsMeshes[i], _scalesMeshes[i]);
	}
}
...
void Model::LoadMesh(unsigned int indMesh)
{
	...
	std::vector<GLuint> indices = GetIndices(_JSON["accessors"][indAccInd]);
...
{
	// Current node
	json node = _JSON["nodes"][nextNode];
	...
	{
		_translationsMeshes.push_back(translation);
		_rotationsMeshes.push_back(rotation);
		_scalesMeshes.push_back(scale);
		_matricesMeshes.push_back(matNextNode);
...
std::vector<unsigned char> Model::GetData()
{
	...
	bytesText = Utilities::GetFileContents((fileDirectory + uri).c_str());
